...
const getCustomer = async (phone: string): Promise<Customer | null> => {
    ...
    if (row) {
        ...
        return new Customer(phone, row.address, row.id, null, row.has_ordered_before, row.name)
    } else {
        return null
    }
}
...
const getCustomerById = async (id: string): Promise<Customer | null> => {
    ...
    if (row) {
        // Keeping lastUsedAddresses null so they can be fetched later on
        return new Customer(row.phone, row.address, row.id, null, row.has_ordered_before, row.name)
    } else {
        return null
    }
}

const updateCustomerToHavingOrderBefore = async (customer: Customer): Promise<void> => {
...
async function getAllCustomers(): Promise<Customer[]> {
    try {
        ...
        return response.rows?.map((row: any) => new Customer(row.phone, row.address, row.id, null, row.has_ordered_before, row.name)) || [];
    } catch (e) {
        console.log(e)
        return []
    }
...
async function updateCustomerName(firstName: string, lastName: string, customer: Customer) {
    try {
